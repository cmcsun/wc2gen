/***************************************************************************
 *   Copyright (C) 2010  Nikita Sadkov                                     *
 *                                                                         *
...
#define PUD_PLAYER_UNUSED       0
#define PUD_PLAYER_NEUTRAL      2
#define PUD_PLAYER_DISABLED     3
#define PUD_PLAYER_COMPUTER     4
#define PUD_PLAYER_PERSON       5
#define PUD_PLAYER_CAPTURABLE   6
#define PUD_PLAYER_RESCUEABLE   7

#define UNIT_PEASANT      2
#define UNIT_PEON         3
#define UNIT_CRITTER     57
#define UNIT_TOWN_HALL   74
#define UNIT_GREAT_HALL  75
#define UNIT_GOLD_MINE   92
#define UNIT_OIL_PATCH   93
#define UNIT_PLAYER      94

static int tilesetsNum = 4;
enum {SUMMER=0, WINTER, WASTELAND, SWAMP};
static int tilesets[] = {SUMMER, WINTER, WASTELAND};
static int tileset;
...
Unit *addUnit(int type, int owner, int size) {
	Unit *u;
...
static void placeUnits() {
	...
	for(i = 0; i < realH-4; i++) {
		for(j = 0; j < realW-4; j ++) {
			if(!canPlace(j,i,5,0)) {
				...
				placeUnit(j, i, addUnit(UNIT_CRITTER, 0, 1));
				continue;
			}
	...
	for(i = 0; i < realH; i+= SCAN_DIM/4) {
		for(j = 0; j < realW; j += SCAN_DIM/4) {
			...
			race = players[k].race;
			u = addUnit(race==ORC ? UNIT_GREAT_HALL : UNIT_TOWN_HALL, k, 4);
			u->baseCenter = 1;
			placeUnit(hallPlace->x, hallPlace->y, u);

			for(l = 0; l < placesFound; l++) {
				if(places[l].used) continue;
				u = addUnit(race==ORC ? UNIT_PEON : UNIT_PEASANT, k, 1);
...
typedef struct {
  uint8_t Tag[4];
  uint32_t Len;
  uint8_t Data[1]; // of length Len
} chunk;

typedef struct {
  uint8_t Type[8];
  uint32_t Unknown1; // should be 0xFF0A0000
  uint32_t Unknown2; // probably CRC
} pud_header;
...
typedef struct {
  ...
  uint8_t Type;
  uint8_t Owner;
  uint16_t Data; //behaviour or gold_amount/2500
} pud_unit;

#include "template.h"

chunk *newChunk(char *Tag, int Len) {
  chunk *c = (chunk*)malloc(Len+8);
  memset(c, 0, Len+8);
  memcpy(c->Tag, Tag, 4);
  c->Len = Len;
  return c;
...
static void save(char const *filename) {
	int i,j,x,y;
	Player *ps[16]; //players
	FILE *f;
  chunk *c;
#define d1 (c->Data)
#define d2 ((uint16_t*)c->Data)
	...
	memset(ps, 0, sizeof(Player*)*16);
	for(i = 0; i < numPlayers; i++) ps[players[i].num] = &players[i];

  c = newChunk("TYPE", 16);
  pud_header *PH = (pud_header*)c->Data;
  memcpy(PH->Type, "WAR2 MAP", 8);
  PH->Unknown1 = 0xFF0A0000;
  PH->Unknown2 = 0x0FDA00AE;
  saveChunk(f, c);
  ...
  c = newChunk("DESC", 32);
  sprintf(c->Data, "wc2gen map (seed=%d)", seed);
  saveChunk(f, c);

  c = newChunk("OWNR", 16);
	for(i = 0; i < 16; i++) {
    if (i < 8) {
      if(!ps[i]) c->Data[i] = PUD_PLAYER_UNUSED;
      else if(ps[i]->human) c->Data[i] = PUD_PLAYER_PERSON;
      else c->Data[i] = PUD_PLAYER_COMPUTER;
    } else {
      if (i == 15) c->Data[i] = PUD_PLAYER_NEUTRAL;
      else c->Data[i] = PUD_PLAYER_DISABLED;
    }
	}
  saveChunk(f, c);

  c = newChunk("ERA ", 2); //SWAMP tileset would require ERAX tag instead
  d2[0] = tileset;
  saveChunk(f, c);
  ...
  c = newChunk("UDTA", sizeof(UDTA)); //unit properties
  memcpy(c->Data, UDTA, sizeof(UDTA));
  saveChunk(f, c);

  c = newChunk("UGRD", sizeof(UGRD)); //upgrade properties
  memcpy(c->Data, UGRD, sizeof(UGRD));
  saveChunk(f, c);

  c = newChunk("SIDE", 16); //player sides
  for(i = 0; i < 16; i++) d1[i] = ps[i] ? ps[i]->race : 0;
  saveChunk(f, c);

  // starting conditions...
  c = newChunk("SGLD", 32);
  for(i = 0; i < 16; i++) d2[i] = ps[i] ? ps[i]->gold : 1000;
  saveChunk(f, c);

  c = newChunk("SLBR", 32);
  for(i = 0; i < 16; i++) d2[i] = ps[i] ? ps[i]->wood : 1000;
  saveChunk(f, c);

  c = newChunk("SOIL", 32);
  for(i = 0; i < 16; i++) d2[i] = ps[i] ? ps[i]->oil : 1000;
  saveChunk(f, c);

  c = newChunk("AIPL", 16); //player AIs
  for(i = 0; i < 16; i++) d1[i] = 0; //0=active, 1=passive
  saveChunk(f, c);

  c = newChunk("MTXM", realW*realH*2); //tile indices
  i = 0;
	for(y = 0; y < realH; y++) {
		for(x = 0; x < realW; x++) {
			Tile *t = xy(x,y)->tile;
      d2[i++] = t->ids[lcg()%t->idsUsed]; //pick one of variations
		}
  }
  saveChunk(f, c);

  //legacy from early game, where oil for example was part of the map
  c = newChunk("SQM ", realW*realH*2);
  i = 0;
	for(y = 0; y < realH; y++) {
		for(x = 0; x < realW; x++) {
      d2[i++] = 0x01; //usually 0x01, but saw 0x11, 0x40, 0x81, 0x82, 0x89
		}
  }
  saveChunk(f, c);

  c = newChunk("OILM", realW*realH);
  i = 0;
	for(y = 0; y < realH; y++) {
		for(x = 0; x < realW; x++) {
      d1[i++] = 0x00;
  ...
  c = newChunk("UNIT", (unitsUsed+numPlayers-1)*8);
  pud_unit *PU = (pud_unit*)c->Data;
  j = 0;
  for(i = 1; i<numPlayers; i++) {
    pud_unit *pu = &PU[j++];
    pu->Type = UNIT_PLAYER;
    pu->X = players[i].x;
    pu->Y = players[i].y;
    pu->Owner = players[i].num;
  }
	for(i = 0; i < unitsUsed; i++) {
		Unit *u = &units[i];
		if(!u->cell) {
			printf("save(): unit %d got no location!\n", u->type);
			continue;
		}
    pud_unit *pu = &PU[j++];
    pu->Type = u->type; //u->id
    pu->X = u->cell->x;
    pu->Y = u->cell->y;
    pu->Owner = players[u->owner].num;
    if (u->resources) pu->Data = u->resources/2500;
    else pu->Data = 0;
	}
  saveChunk(f, c);
...
static void usage() {
  printf("Usage:"); 
  printf("  wc2gen [Options...] output_filename\n");
  printf("\n");
  printf("Options:\n");
  printf("  -s, --seed=N                 Map seed\n");
  printf("  -r, --race=N                 Player's side\n");
  printf("  -p, --players=N              Number of players\n");
  printf("  -g, --gold=N                 Starting gold\n");
  printf("  -o, --oil=N                  Starting oil\n");
  printf("  -l, --landmass=N             Percent of land\n");
  printf("  -e, --emitters=N             Number of islands\n");
  printf("Example:\n");
  printf("  wc2gen --race=orc --players=6 -emitters=2 output.pud\n");
  printf("    map with 6 players and 2 islands for an orc player\n");
}
...
int main(int argc, char **argv) {
  ...
  srand(time(0));
  seed=rand();
  int race=HUMAN, players=4, gold=5000, oil=2500, landmass=80, numLandEmitters=1;
  struct option Options[] = {
    ...
    {"oil", required_argument, 0, 'o'},
    {"landmass", required_argument, 0, 'l'},
    {"emitters", required_argument, 0, 'e'},
    {NULL, no_argument, NULL, 0 }};

  while((op = getopt_long(argc, argv, "o:s:r:p:g:o:l:e:", Options, NULL)) != -1) {
    if (!optarg && op) {printf("-%c requires an argument\n", op); return -1;}
    switch(op) {
    case 0: break; // long option already processed by getopt_long
    case 's': seed = strtoul(optarg, 0, 10); break;
    case 'r': race = !strcmp(strdup(optarg),"human") ? HUMAN : ORC; break;
    case 'p': players = atoi(optarg); break;
    case 'g': gold = atoi(optarg); break;
    case 'o': oil = atoi(optarg); break;
    case 'l': landmass = atoi(optarg); break;
    case 'e': numLandEmitters = atoi(optarg); break;
    }
  }
  if (optind < argc) filename = strdup(argv[optind++]);
  if (!filename) {
     usage();
     return -1;
  }
  generate_map(filename, seed, race, players, gold, oil, landmass, numLandEmitters);
}
